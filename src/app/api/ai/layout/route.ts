...
import { generateLayout } from "@/lib/ai-service";
import { aiLayoutSchema } from "@/lib/validators/ai";
...
export async function POST(req: NextRequest) {
  ...
  const limit = rateLimit({
    key: buildRateLimitKey(["ai", "layout", auth.user.id]),
    limit: 5,
    windowMs: 60_000,
  });
  if (!limit.allowed) {
  ...
  try {
    const body = aiLayoutSchema.parse(await req.json());
    if (body.project_id) {
    ...
    const { output, usage } = await generateLayout(body.prompt, body.context ?? "");
