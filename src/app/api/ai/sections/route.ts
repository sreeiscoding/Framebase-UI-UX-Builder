...
import { generateSections } from "@/lib/ai-service";
import { aiSectionsSchema } from "@/lib/validators/ai";
...
export async function POST(req: NextRequest) {
  ...
  const limit = rateLimit({
    key: buildRateLimitKey(["ai", "sections", auth.user.id]),
    limit: 5,
    windowMs: 60_000,
  });
  if (!limit.allowed) {
  ...
  try {
    const body = aiSectionsSchema.parse(await req.json());
    if (body.project_id) {
    ...
    const { output, usage } = await generateSections(body.data);

    await auth.supabase.from("ai_generations").insert({
      user_id: auth.user.id,
      project_id: body.project_id ?? null,
      generation_type: "sections",
      input_prompt: body.data,
